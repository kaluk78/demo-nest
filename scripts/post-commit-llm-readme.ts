...
interface ScriptConfig {
  timeout: number; // milliseconds (0 means no timeout)
  maxRetries: number;
  logLevel: 'info' | 'verbose' | 'silent';
}

const config: ScriptConfig = {
  // Use 0 to disable execSync timeout so post-commit processing can finish
  timeout: 0,
  maxRetries: 3,
  logLevel: 'info'
};
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (error.code === 'ETIMEDOUT') {
        ...
        // Only retry on timeouts; assume other errors are not transient
        const waitTime = Math.pow(2, attempt - 1) * 1000;
        logger.verbose(`Waiting ${waitTime}ms before retry after timeout...`);
      ...
      // For non-timeout errors, do not retry – fail fast so we don't restart long-running commands
      logger.warn(`Command failed (non-timeout) on attempt ${attempt}/${retries}: ${error.message}`);
